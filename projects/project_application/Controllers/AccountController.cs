...
using project_application.Filters;
...
namespace project_application.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(LoginModel model, string returnUrl)
        {
            ...
            {
                ModelState.AddModelError("", "对不起，你的账号尚未验证通过。");
            }
            else
            {
        ...
        public ActionResult Activate()
        {
            using (var db = new UsersContext())
            {
                var accounts = (from u in db.UserProfiles
                                select new ActivateModel()
                                          {
                                              UserId = u.UserId,
                                              UserName = u.UserName,
                                              DepartmentName = u.UserDepartment.DepartmentName,
                                              RealName = u.RealName
                                          }).ToList();
                var accountsToActivate = new List<ActivateModel>();
                foreach(var account in accounts)
                {
                    if (WebSecurity.IsConfirmed(account.UserName))
                        continue;
                    account.Status = "未激活";
                    accountsToActivate.Add(account);
                }
                return View(accountsToActivate);
            }
        }
