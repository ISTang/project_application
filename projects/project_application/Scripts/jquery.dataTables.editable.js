        ...
        function _fnOnRowAdded(data) {
            ...
            if (properties.fnOnNewRowPosted(data)) {
                ...
                $("input:text[rel],input:radio[rel][checked],input:hidden[rel],select[rel],textarea[rel],span.datafield[rel]", oAddNewRowForm).each(function () {
                    ...
                    var sCellValue = "";
                    if (rel >= iColumnCount)
                        properties.fnShowError("增加表单中，名字为 '" + $(this).attr("name") + "' 且带有 'rel' 属性的输入元素的个数不能多于列数 - " + iColumnCount, "add");
                    else {
        ...
        return this.each(function () {
            ...
            if (oAddDeleteToolbar.length != 0) {
                if (oAddNewRowButton == null && properties.sAddNewRowButtonId != ""
                    && oAddNewRowForm != null) {
                    oAddDeleteToolbar.append("<button id='" + properties.sAddNewRowButtonId + "' class='add_row'>增加</button>");
                    oAddNewRowButton = $("#" + properties.sAddNewRowButtonId);
                    oAddNewRowButton.click(function () { oAddNewRowForm.dialog('open'); });
                }
                if (oDeleteRowButton == null && properties.sDeleteRowButtonId != "") {
                    oAddDeleteToolbar.append("<button id='" + properties.sDeleteRowButtonId + "' class='delete_row'>删除</button>");
                    oDeleteRowButton = $("#" + properties.sDeleteRowButtonId);
                    oDeleteRowButton.click(_fnOnRowDelete);
                }
            }
